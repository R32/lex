...
var Demo = function() { };
Demo.main = function() {
  console.log("Demo.hx:8:",Parser._entry(new Parser(new Lexer("1 + 2 + 3 + -1 + \"abc\\\"\"")).stream,0,9));
};
...
Parser.cases = function(f,s) {
  ...
  case 7:
    ...
    return n;
  default:
    var str = Parser.unescape(s.lex.input,s.cached[s.pos + -1].pmin,s.cached[s.pos + -1].pmax);
    s.reduce(10,1);
    return str.length;
  }
};
Parser.unescape = function(input,pmin,pmax) {
  return input.substr(pmin + 1,pmax - pmin - 2);
};
