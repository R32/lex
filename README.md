...
Parser._entry = function(stream,state,exp) {
    ...
    t = stream.cached[stream.pos + -1];
    var p = lm_Utils.line(stream.lex.input,t.pmin);
    throw new Error("Unexpected \"" + (t.term != 0 ? stream.lex.getString(t.pmin,t.pmax - t.pmin) : "Eof") + "\" at line: " + (p & 65535) + ", char: " + (p >>> 16));
};
...
lm_Utils.line = function(input,pmin) {
    var line = 1;
    var char = 0;
    var i = 0;
